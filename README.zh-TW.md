...
給它一次對話，或同一件工作被重開的好幾次對話。它會讀完整份紀錄，包括你的發言和每一次工具呼叫。每當你**糾正或介入** Claude，它都問一句：*這個錯本來可以在哪一層被擋下？*
...
它也會找**沉默的失敗**。沒人糾正，不代表沒出錯，可能只是沒人去檢查。所以不管你有沒有糾正過，它都會找：

- Claude 說「完成」「測試通過」「已確認」，但對話裡找不到對應的證據
- 會把錯誤藏起來的指令寫法，例如 `|| true`、`2>/dev/null`、`| tail`
- 資料被過濾或刪除後，沒有印出前後筆數
- 你交代的步驟被悄悄省略或縮水

找到一處，就回頭搜整份紀錄，看同樣的寫法還出現過幾次，因為被發現的往往不是唯一一次。沒問題的報告只會是兩種說法之一：「查過了，確實沒問題」，或「沒人糾正，但有 N 處宣稱沒有證據」，不會只寫「沒問題」。
...
最後檢查**目標對齊**：代理知不知道工作該往哪裡走？它會找出專案的目標寫在哪、是誰寫的（你自己、外部交接文件，還是代理自己推論的），和你親口說的目標是否一致，目前的優先順序由誰決定，以及有沒有辦法看出離完成還有多遠。一個專案可能每件事都做對，卻因為方向是代理自己定的而越走越偏；錯誤修得再乾淨，方向錯了一樣是在原地打轉。
...
> 「從你糾正、介入代理的那一刻開始想」是她的說法；額外的訊號（Claude 自己撤回、工具錯誤、宣稱沒驗證）整個「沉默的失敗」檢查，以及「目標對齊」檢查，是本技能自己加的延伸。
...
1. 內附腳本把對話紀錄壓成一份純文字摘要，放在暫存資料夾：保留完整對話、縮短工具呼叫、丟掉圖片、遮蔽看起來像金鑰的字串，並標出可能把錯誤藏起來的指令。
...
- 「疑似糾正」和「可能吞掉錯誤」標記只是字串比對，會漏掉真正的問題，也會誤標無害的寫法，例如只是為了縮短輸出而用的 `| head` 也會被標到。
